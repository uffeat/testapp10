...
components.factories.add(
  ...
  (parent) => {
    ...
    const cls = class Shadow extends parent {
      #current_children = new Set();
      #previous_children = new Set();
      #set_has_children;
      #set_has_content;
      constructor(...args) {
        super(...args);
        ...
        slot.addEventListener("slotchange", (event) => {
          this.#previous_children = this.#current_children;
          this.#current_children = new Set([...this.children]);
          //console.log('current:', [...this.#current_children]);
          //console.log('previous:', [...this.#previous_children]);

          const added = this.#current_children.difference(
            this.#previous_children
          );
          const removed = this.#previous_children.difference(
...
components.factories.add(
  ...
  (parent) => {
    ...
    const cls = class ChildObserver extends parent {
      #child_mutation_handler = (mutations) => {
        mutations.forEach((mutation) => {
          if (mutation.type === "childList") {
            [...mutation.addedNodes]
              .filter((node) => node instanceof HTMLElement)
              .forEach((node) => {
                node.send("added_to_parent", { detail: { new_parent: this } });
                this.send("child_added", { detail: { added_child: node } });
              });
            [...mutation.removedNodes]
              .filter((node) => node instanceof HTMLElement)
              .forEach((node) => {
                node.send("removed_from_parent", {
                  detail: { previous_parent: this },
                });
                this.send("child_removed", { detail: { removed_child: node } });
              });
          }
        });
      };
      #child_observer = new MutationObserver(this.#child_mutation_handler);
      #observing_children = false;
      ...
      connectedCallback() {
        super.connectedCallback && super.connectedCallback();
        this.observe_children(true);
      }

      disconnectedCallback() {
        super.disconnectedCallback && super.disconnectedCallback();
        this.observe_children(false);
      }

      

      observe_children = (observe = true) => {
        if (observe) {
          if (!this.#observing_children) {
            this.#child_observer.observe(this, {
              childList: true,
              subtree: false,
            });
            this.#observing_children = true;
          }
        } else {
          this.#child_observer.disconnect();
          this.#observing_children = false;
        }
      };
